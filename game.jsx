...
// ── 紙娃娃（立繪分級）系統 ───────────────────────────────────────────
// 規則：立繪必須「完全符合」對應穿著才顯示；不成套則顯示通用預設立繪 keyuFallback。
//   1) PORTRAIT_COMBOS：穿著指定組合（need 必穿 + empty 必空）完全命中 → 該立繪。例：比基尼。
//   2) PORTRAIT_TIERS：6 個主要部位(上著/內衣/下著/內褲/襪子/鞋子)全部穿著且同為第 N 級 → 第 N 級立繪。
//   3) 以上皆不符（缺件、混級、混主題）→ keyuFallback（通用預設）。
const PORTRAIT_TIERS = { 1: keyuT1, 2: keyuT2, 3: keyuT3, 4: keyuT4, 5: keyuT5, 6: keyuT6, 7: keyuT7, 8: keyuT8, 9: keyuT9, 10: keyuT10, 11: keyuT11, 12: keyuT12, 13: keyuT13 };
// 特殊組合立繪（依序比對，need+empty 完全滿足才回傳）
const PORTRAIT_COMBOS = [
  // 比基尼：內衣 b14 + 內褲 p14，且上衣/下著/襪子/鞋子皆為空，才顯示
  { img: keyuT14, need: { bra: 'b14', panties: 'p14' }, empty: ['top', 'bottom', 'socks', 'shoes'] },
];
const PORTRAIT_MAIN_SLOTS = ['top', 'bra', 'bottom', 'panties', 'socks', 'shoes'];
const pickPortrait = (clothes) => {
  const c = clothes || {};
  const idOf = (it) => (it && it.id) || null;
  // 1) 特殊組合
  for (const combo of PORTRAIT_COMBOS) {
    const needOk = Object.entries(combo.need).every(([slot, id]) => idOf(c[slot]) === id);
    const emptyOk = (combo.empty || []).every(slot => !c[slot]);
    if (needOk && emptyOk) return combo.img;
  }
  // 2) 整套同級：6 部位全穿、且 tier 全部相同、且該級有立繪
  const tierOf = (it) => (it && /(\d+)$/.test(it.id)) ? parseInt(it.id.match(/(\d+)$/)[1], 10) : 0;
  const tiers = PORTRAIT_MAIN_SLOTS.map(s => tierOf(c[s]));
  if (tiers.every(t => t > 0 && t === tiers[0]) && PORTRAIT_TIERS[tiers[0]]) {
    return PORTRAIT_TIERS[tiers[0]];
  }
  // 3) 不成套 → 通用預設立繪
  return keyuFallback;
};
...
const CLOTHING_DB = {
  ...
  bra: [
    ...
    ['b5', '豹紋深V托高內衣',        56, 1700],
    ['b6', '紅色蕾絲深V內衣',         72, 2500],
    ['b7', '香檳金絲緞托高內衣',         90, 3600],
    ['b8', '黑色蕾絲深V內衣',        110, 4900],
    ['b9', '黑色絲綢肚兜內衣',      132, 6800],
    ['b10','無肩隱形胸貼',         157, 9000],
    ['b11','酒紅蕾絲半罩內衣',        185,12500],
    ['b12','黑色透視蕾絲內衣',      215,17500],
    ['b13','黑色哥德蕾絲內衣', 248, 24000],
    ['b14','紅色比基尼罩杯', 285, 33000],
  ...
  panties: [
    ...
    ['p12','黑色透視蕾絲丁字褲',      242,17500],
    ['p13','黑色哥德蕾絲內褲', 280, 24000],
    ['p14','紅色細帶比基尼丁字褲', 320, 33000],
  ...
  socks: [
    ...
    ['sk2', '白色蕾絲短襪',           8,  225],
    ['sk3', '黑色絲質膝上襪',        14,  400],
    ['sk4', '純白蕾絲吊帶膝上襪',        22,  650],
    ['sk5', '黑色魚網大腿襪',        32, 1000],
    ['sk6', '紅色蕾絲吊帶大腿襪',        44, 1500],
    ['sk7', '膚色透膚絲襪',        58, 2250],
    ['sk8', '黑色蕾絲頂大腿襪',      74, 3250],
    ['sk9', '黑色透膚大腿襪',      92, 4500],
    ['sk10','裸膚隱形絲襪',       112, 6250],
    ['sk11','酒紅蕾絲吊帶大腿襪',   135, 8500],
    ['sk12','黑色蕾絲吊帶大腿襪',   160,12000],
    ['sk13','黑色哥德蕾絲大腿襪', 188, 16500],
  ].map(([id,name,charm,price])=>({id,name,charm,price,type:'socks'})),
  shoes: [
    ['sh1', '裸色細跟低跟鞋',          6,  200],
    ['sh2', '黑色細跟包頭鞋',         12,  450],
    ['sh3', '白色細跟高跟鞋',         20,  850],
    ['sh4', '純白緞面蝴蝶結高跟鞋',         30, 1400],
    ['sh5', '黑色漆皮細跟高跟鞋',         42, 2100],
    ['sh6', '紅色漆皮尖頭高跟鞋',         56, 3000],
    ['sh7', '藏青漆皮尖頭高跟鞋',         72, 4250],
    ['sh8', '黑色繫踝尖頭高跟鞋',     90, 6000],
    ['sh9', '黑色尖頭細跟高跟鞋',       110, 8250],
    ['sh10','銀色繞踝綁帶高跟',      132,11000],
    ['sh11','酒紅絲緞細跟高跟鞋',      156,15000],
    ['sh12','黑色繞踝魚口細跟涼鞋',183,21000],
    ['sh13','黑色哥德厚底繫帶踝靴', 213, 29000],
