...
// ── 紙娃娃（立繪分級）系統 ───────────────────────────────────────────
// 兩種對應方式：
//   1) PORTRAIT_COMBOS：穿著「指定單品組合」即顯示對應立繪（優先、不靠分級）。
//      例：比基尼只需內衣 b14 + 內褲 p14，其餘部位不限。
//   2) PORTRAIT_TIERS：「整套同級」對應——主要服裝(上著/內衣/下著/內褲/襪子/鞋子)全部達第 N 級則顯示第 N 級立繪。
...
// 特殊組合立繪（依序比對，完全命中才回傳）：
//   need  = 必須穿著的 slot→單品id；empty = 必須為空（沒穿）的 slot。
//   兩者皆須完全滿足才顯示該立繪，否則不顯示（落回分級邏輯）。
...
const pickPortrait = (clothes) => {
  ...
  const outfitTier = Math.min(...main.map(s => tierOf(c[s])));
