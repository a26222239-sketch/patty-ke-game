...
// 特殊組合立繪（依序比對，命中即回傳）：need 列出必須穿著的 slot→單品id
const PORTRAIT_COMBOS = [
  { img: keyuT14, need: { bra: 'b14', panties: 'p14' } }, // 比基尼：穿內衣+內褲即顯示
];
const pickPortrait = (clothes) => {
  const c = clothes || {};
  const idOf = (it) => (it && it.id) || null;
  for (const combo of PORTRAIT_COMBOS) {
    if (Object.entries(combo.need).every(([slot, id]) => idOf(c[slot]) === id)) return combo.img;
  }
